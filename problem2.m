%Time invarient singal of frequency 300hz and 450hz
close all; clc; clear;
observe_time = 1;
sample_freq = 1024;
...
for i = 1:n_sample_pt
    t = dt*(i-1);
    signal(i) = a1 * sin(2*pi*f1*t) + a2 * sin(2*pi*f2*t);
end
...
subplot(3, 1, 3);
spectrogram(signal, hann(8), [], [], sample_freq, "yaxis"); %not giving argument noverlap and f
title("Spectrogram")
ylabel("Frequency")
xlabel("Time (ms)")
